...
def import_truth():
    ...
    try:
        ...
        for pnum, fen in ground_truth.items():
            puzzle = db.query(Puzzle).filter_by(chapter_id=1, puzzle_number=pnum).first()
            if puzzle:
                puzzle.fen = fen
                # Re-validate against existing solution
                import chess
                try:
                    board = chess.Board(fen)
                    if puzzle.solution_san:
                        move = board.parse_san(puzzle.solution_san)
                        puzzle.solution_uci = move.uci()
                        puzzle.extraction_confidence = 1.0
                        puzzle.error_message = None
                        puzzle.verified = 1 # Mark as verified ground truth
                        print(f"Updated and validated puzzle {pnum}")
                    else:
                        print(f"Updated FEN for puzzle {pnum}, but no solution found yet.")
                except Exception as e:
                    print(f"Puzzle {pnum}: FEN updated but validation failed: {e}")
                updated += 1
        db.commit()
        print(f"Finished. Updated {updated} puzzles.")
    finally:
        db.close()
